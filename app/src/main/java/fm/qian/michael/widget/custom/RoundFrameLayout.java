...
public class RoundFrameLayout extends FrameLayout {
    ...
    public RoundFrameLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        if (attrs != null) {
            TypedArray ta = context.obtainStyledAttributes(attrs, fm.qian.michael.R.styleable.RoundFrameLayout);
            float radius = ta.getDimension(fm.qian.michael.R.styleable.RoundFrameLayout_rfl_radius, 0);
            mTopLeftRadius = ta.getDimension(fm.qian.michael.R.styleable.RoundFrameLayout_topLeftRadius, radius);
            mTopRightRadius = ta.getDimension(fm.qian.michael.R.styleable.RoundFrameLayout_topRightRadius, radius);
            mBottomLeftRadius = ta.getDimension(fm.qian.michael.R.styleable.RoundFrameLayout_bottomLeftRadius, radius);
            mBottomRightRadius = ta.getDimension(fm.qian.michael.R.styleable.RoundFrameLayout_bottomRightRadius, radius);
            ta.recycle();
        }
