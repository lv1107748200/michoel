...
public class SelectableRoundedImageView extends android.support.v7.widget.AppCompatImageView {
    ...
    public SelectableRoundedImageView(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs,
                       fm.qian.michael.R.styleable.SelectableRoundedImageView, defStyle, 0);

        final int index = a.getInt(fm.qian.michael.R.styleable.SelectableRoundedImageView_android_scaleType, -1);
        if (index >= 0) {
            setScaleType(sScaleTypeArray[index]);
        }

        mLeftTopCornerRadius = a.getDimensionPixelSize(
                                   fm.qian.michael.R.styleable.SelectableRoundedImageView_sriv_left_top_corner_radius, 0);
        mRightTopCornerRadius = a.getDimensionPixelSize(
                                    fm.qian.michael.R.styleable.SelectableRoundedImageView_sriv_right_top_corner_radius, 0);
        mLeftBottomCornerRadius = a.getDimensionPixelSize(
                                      fm.qian.michael.R.styleable.SelectableRoundedImageView_sriv_left_bottom_corner_radius, 0);
        mRightBottomCornerRadius = a.getDimensionPixelSize(
                                       fm.qian.michael.R.styleable.SelectableRoundedImageView_sriv_right_bottom_corner_radius, 0);
        ...
        mBorderWidth = a.getDimensionPixelSize(fm.qian.michael.R.styleable.SelectableRoundedImageView_sriv_border_width, 0);
        if (mBorderWidth < 0) {
            throw new IllegalArgumentException("border width cannot be negative.");
        }

        mBorderColor = a.getColorStateList(fm.qian.michael.R.styleable.SelectableRoundedImageView_sriv_border_color);
        if (mBorderColor == null) {
            mBorderColor = ColorStateList.valueOf(DEFAULT_BORDER_COLOR);
        }

        isOval = a.getBoolean(fm.qian.michael.R.styleable.SelectableRoundedImageView_sriv_oval, false);
        a.recycle();
